...
// ─── FIREBASE SDK (loaded via CDN in index.html – here we use compat via window) ───
// We'll use the Anthropic API for the Claude-in-Claude features and
// Firebase via dynamic import approach using script tags injected at runtime.

// ─── QUESTION BANK (from Hawaii Driver's Manual) ───────────────────────────
const QUESTIONS = [
  ...
  { id: 14, q: "You don't say much after your boss chews you out, but you think he is unfair and you are angry. When you get to your car you should:", opts: ["Play the radio loud so you won't think about it.", "Drive fast on the Interstate so you can let off steam.", "Take a few minutes to cool off before you drive home.", "Stop in at your favorite bar and have a few drinks before driving home."], ans: 2, page: 24 },
  ...
  { id: 26, q: "You are driving on the roadway and there is an emergency vehicle with siren and flashing lights directly behind you in heavy traffic. You should:", opts: ["Stop.", "Blow your horn.", "Force your way into traffic in the adjoining lane.", "Keep moving slowly until you can get out of the way."], ans: 3, page: 29 },
  ...
  { id: 44, q: "A yellow 'X' over the traffic lane means:", opts: ["That you must move out of that lane of traffic because it is going to be used for oncoming traffic.", "That you may use that lane.", "That the lane is for left turns only.", "That you may use the lane but with caution."], ans: 0, page: 42 },
  ...
  { id: 51, q: "Defensive driving is a driving technique in which you:", opts: ["Identify dangerous driving situations and take action to avoid a crash.", "Defend against poor drivers by getting ahead of them in traffic.", "Defend yourself against crashes by following close to the vehicle ahead.", "Drive fast enough to stay ahead of traffic congestion."], ans: 0, page: 59 },
  ...
  { id: 56, q: "Following too closely is listed as the cause of many traffic crashes. The proper following interval is:", opts: ["Close enough so no one will cut in front of you.", "At least 2 seconds behind the vehicle ahead.", "100 feet.", "75 feet."], ans: 1, page: 67 },
  ...
  { id: 63, q: "When towing a loaded trailer weighing more than half the towing vehicle's weight or 3,000+ pounds, the trailer must be equipped with:", opts: ["Brakes.", "Breakaway protection.", "Hub caps.", "Brakes and breakaway protection."], ans: 3, page: 87 },
  ...
  { id: 70, q: "In Hawaii, pedestrians crossing against the red include those:", opts: ["Who have no responsibility for their own safety.", "Whom you must still stop for if they are in the crosswalk.", "Who always have the right of way over vehicles.", "Who cannot be held liable for causing crashes."], ans: 1, page: 32 },
...
const FIREBASE_CONFIG = {
  apiKey: "demo-key",
  authDomain: "hawaii-permit-prep.firebaseapp.com",
  projectId: "hawaii-permit-prep",
};

// ─── LOCAL STORAGE HELPERS (Firebase substitute for artifact context) ─────────
const STORAGE_KEY = "hawaii_permit_data";
...
function isDue(stat) {
  if (!stat) return true;
  const daysSince = (Date.now() - (stat.lastSeen || 0)) / 86400000;
  return daysSince >= getLeitnerInterval(stat.box || 0);
}

// ─── PASSING PREDICTOR ───────────────────────────────────────────────────────
function computePredictor(sessions) {
  if (sessions.length === 0) return { current: null, trend: "none", projected: null };
  ...
  if (recent.length >= 2) {
    ...
    const f = first.reduce((a, b) => a + b, 0) / first.length;
    const l = last.reduce((a, b) => a + b, 0) / last.length;
    if (l - f > 5) trend = "improving";
    else if (f - l > 5) trend = "declining";
  }
  // project future score with simple linear regression
  let projected = avg;
  if (sessions.length >= 3) {
    const scores = sessions.map((s, i) => ({ x: i, y: (s.correct / s.total) * 100 }));
    ...
    projected = Math.min(100, Math.max(0, (sumY / n) + slope * 2));
  }
  return { current, avg, trend, projected, sessions: sessions.length };
}
...
function selectQuestions(mode, stats, count = 10) {
  ...
  switch (mode) {
    ...
    case "growth": {
      // Questions with lowest accuracy (hardest)
    ...
    case "spaced": {
      // Due questions by Leitner box
      const due = QUESTIONS.filter(q => isDue(stats[q.id]));
      if (due.length === 0) return QUESTIONS.sort(() => Math.random() - 0.5).slice(0, count);
      return due.sort(() => Math.random() - 0.5).slice(0, count);
    }
    default:
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    --warning: #f39c12;
    --radius: 12px;
    --shadow: 0 4px 24px rgba(10,34,64,0.18);
  ...
  .pred-bar-label { font-size: 0.65rem; color: rgba(245,230,200,0.6); text-align: center; }
  .pred-bar-val { font-size: 0.75rem; font-weight: 500; color: var(--sand); text-align: center; }
  .pred-pass-line {
    border-top: 2px dashed var(--coral);
    position: relative;
    margin: 8px 0;
  }
  .pred-pass-line::after {
    content: 'PASSING (70%)';
    position: absolute;
    right: 0; top: -9px;
    font-size: 0.6rem;
  ...
  .nav-tab:hover { border-color: var(--ocean-light); background: var(--sand); }
  .nav-tab.active {
    background: var(--ocean);
    color: white;
  ...
  .account-sub { font-size: 0.8rem; color: var(--muted); margin-bottom: 20px; line-height: 1.5; }
  ...
  .firebase-note strong { color: var(--ocean); }
  ...
  .hc-pass-line { position: relative; margin-bottom: 4px; }
...
export default function App() {
  ...
  const [userData, setUserData] = useState(() => loadData() || initUserData());
  const [quizState, setQuizState] = useState(null); // null | active
  const [selectedMode, setSelectedMode] = useState("random");
  const [quizCount, setQuizCount] = useState(10);
  const [username, setUsername] = useState(() => localStorage.getItem("hawaii_username") || "");
  ...
  const updateStats = useCallback((results) => {
    setUserData(prev => {
      ...
      results.forEach(r => {
        ...
        const isCorrect = r.userAns === r.ans;
        newStats[r.id] = {
          attempts: s.attempts + 1,
          correct: s.correct + (isCorrect ? 1 : 0),
          box: isCorrect ? Math.min((s.box || 0) + 1, 5) : 0,
          lastSeen: Date.now(),
        };
      });
  ...
  const handleAnswer = (optIdx) => {
    ...
    setQuizState(prev => ({ ...prev, feedback: { chosen: optIdx, correct: isCorrect } }));
  };
  ...
  return (
    <>
      <style>{css}</style>
      <div style={{ background: "var(--ocean)", paddingBottom: 0 }}>
        <div className="header">
          <div className="header-inner">
            <div className="header-logo">
              <span>🌊</span> Hawaii <span>Permit Prep</span>
            </div>
        ...
        <div className="hero">
          <div className="hero-content">
            ...
            <p>Adaptive prep for the Hawaii Driver's Knowledge Test</p>
          </div>
      ...
      <div className="app">
        ...
        <div className="stats-row">
          <div className="stat-card">
            ...
            <div className="stat-value">{userData.sessions.length}</div>
            <div className="stat-sub">total quizzes taken</div>
          </div>
          <div className="stat-card">
          ...
          <div className="stat-card">
            ...
            <div className="stat-value">{userData.totalAttempts}</div>
            <div className="stat-sub">total attempts</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">Mastered</div>
            <div className="stat-value">{leitnerCounts[4] + leitnerCounts[5]}</div>
            <div className="stat-sub">of {QUESTIONS.length} questions</div>
          </div>
        </div>
        ...
        {userData.sessions.length > 0 && (
          <div className="predictor-card">
            ...
            {(() => {
              const last5 = userData.sessions.slice(-5);
              const maxH = 70;
              return (
                  ...
                  {last5.map((s, i) => (
                    <div className="pred-bar-wrap" key={i}>
                      ...
                      <div
                        className="pred-bar"
                        style={{
                          height: `${Math.round((s.pct / 100) * maxH)}px`,
                          background: s.pct >= 70 ? "var(--success)" : s.pct >= 50 ? "var(--warning)" : "var(--danger)"
                        }}
                      />
                      <div className="pred-bar-label">#{userData.sessions.length - last5.length + i + 1}</div>
                    </div>
                  ))}
                  {predictor.projected !== null && (
                    <div className="pred-bar-wrap">
                      ...
                      <div
                        className="pred-bar"
                        style={{
                          height: `${Math.round((predictor.projected / 100) * maxH)}px`,
            ...
            <div className="pred-trend">
              ...
              {predictor.trend === "declining" && "📉 Score declining — try Growth mode."}
              {predictor.trend === "stable" && "➡️ Score is stable — push for Growth mode."}
              {predictor.projected !== null && (
        ...
        <div className="bottom-nav">
          {[
            ...
            <div
              ...
              onClick={() => { if (t.id !== "quiz") setTab(t.id); }}
            >
          ...
          <>
            {userData.sessions.length === 0 && (
              <div className="empty-state">
                ...
                <p>Welcome to Hawaii Permit Prep! Head to <strong>Study</strong> to take your first quiz. Your stats and progress will appear here.</p>
              </div>
            )}
            {userData.sessions.length > 0 && (
              <>
                    ...
                    {userData.sessions.slice(-15).map((s, i) => (
                      <div className="hc-bar-col" key={i}>
                        <div
                          className="hc-bar"
                          style={{
                            height: `${Math.round((s.pct / 100) * 90)}px`,
                            background: s.pct >= 70 ? "var(--success)" : s.pct >= 50 ? "var(--warning)" : "var(--danger)"
                          }}
                          title={`Session ${i + 1}: ${s.pct}%`}
                        />
                        <div className="hc-bar-num">{s.pct}%</div>
                      </div>
                    ))}
                  </div>
                  <div style={{ fontSize: "0.7rem", color: "var(--muted)", marginTop: "8px", textAlign: "right" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
                  {userData.sessions.slice(-5).reverse().map((s, i) => (
                    <div key={i} style={{ background: "white", borderRadius: 8, padding: "12px 16px", boxShadow: "0 2px 8px rgba(10,34,64,0.08)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
        ...
        {tab === "study" && !quizState && (
          <>
            ...
            <div className="start-quiz-section">
              <select
                className="q-count-select"
                value={quizCount}
                onChange={e => setQuizCount(Number(e.target.value))}
              >
                {[5, 10, 15, 20, 25, 40].map(n => (
            ...
            <div className="quiz-header">
              <div className="quiz-progress">
                {quizState.questions.map((_, i) => (
                  <div
                    key={i}
                    className={`qp-dot ${i < quizState.current ? (quizState.answers[i]?.userAns === quizState.answers[i]?.ans ? "correct" : "wrong") : i === quizState.current ? "current" : ""}`}
                  />
                ))}
              </div>
                  ...
                  <div className="options">
                    {q.opts.map((opt, i) => {
                      ...
                      if (fb !== null) {
                        ...
                        else if (i === fb.chosen && i !== q.ans) cls = "wrong";
                      }
                      return (
                        <button
                          key={i}
            ...
            {quizState.feedback && (
              <>
                <div className={`feedback-bar ${quizState.feedback.correct ? "correct" : "wrong"}`}>
                  {quizState.feedback.correct ? "✅ Correct!" : "❌ Incorrect — study the highlighted answer."}
                </div>
              ...
              return (
                <div className="results-card">
                  ...
                  <div className="results-meta">
                    {correct} of {quizState.answers.length} correct · {["Random","Growth","Easier","Spaced Repetition"][["random","growth","easier","spaced"].indexOf(selectedMode)]} mode
                  </div>
                  <div className="results-breakdown">
                    ...
                    <div className="res-stat">
                      <div className="res-stat-val">{userData.sessions.length}</div>
                      <div className="res-stat-label">Total sessions</div>
                    </div>
                  </div>

                  <div style={{ textAlign: "left", marginTop: 16 }}>
                    <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: "0.95rem", color: "var(--ocean)", marginBottom: 10 }}>
        ...
        {tab === "performance" && (
          <>
            ...
            {Object.keys(userData.questionStats).length === 0 ? (
              ...
              <>
                <div className="section-title">Spaced Repetition Boxes</div>
                      ...
                      <div className="perf-item" key={q.id}>
                        ...
                        <div className="perf-bar-wrap">
                          <div className="perf-bar-fill" style={{
                            width: `${acc}%`,
                            background: acc >= 70 ? "var(--success)" : acc >= 50 ? "var(--warning)" : "var(--danger)"
                          }} />
                        </div>
        ...
        {tab === "account" && (
          <div className="account-panel">
            <div className="account-title">
              {username ? `Aloha, ${username}! 🌺` : "Your Account"}
            </div>
            <div className="account-sub">
              Your progress is saved locally in this browser. To sync across devices, connect your Firebase project below.
            </div>
            ...
            {!username ? (
              <>
                <div className="input-group">
                  ...
                  <input className="input-field" value={nameInput} onChange={e => setNameInput(e.target.value)} placeholder="Your name" />
                </div>
                <button className="btn-primary" onClick={() => {
                  if (nameInput.trim()) {
                    setUsername(nameInput.trim());
                    localStorage.setItem("hawaii_username", nameInput.trim());
                  }
                }}>Save Name</button>
              </>
            ) : (
              <>
                <div style={{ background: "var(--sand)", borderRadius: 8, padding: "14px 16px", marginBottom: 16 }}>
                  <div style={{ fontSize: "0.75rem", color: "var(--muted)", marginBottom: 4 }}>Signed in as</div>
                  <div style={{ fontFamily: "'DM Serif Display', serif", color: "var(--ocean)" }}>{username}</div>
                </div>
                <button className="btn-secondary" onClick={() => {
                  setUsername("");
                  localStorage.removeItem("hawaii_username");
                }}>Change Name</button>
              </>
            )}

            <div style={{ marginTop: 24 }}>
              <button className="btn-secondary" style={{ marginBottom: 10, color: "var(--danger)", borderColor: "var(--danger)" }} onClick={() => {
                if (window.confirm("Reset all progress? This cannot be undone.")) {
                  const reset = initUserData();
                  setUserData(reset);
                  saveData(reset);
                }
            ...
            <div className="firebase-note">
              <strong>🔥 Firebase Setup (for developers)</strong><br /><br />
              To enable cross-device sync with Firebase:<br />
              1. Create a project at <strong>console.firebase.google.com</strong><br />
              2. Enable <strong>Authentication</strong> (Email/Password) and <strong>Firestore</strong><br />
              3. Replace <code>FIREBASE_CONFIG</code> in the source with your config<br />
              4. Install <code>firebase</code> npm package<br />
              5. Replace <code>loadData/saveData</code> with Firestore <code>getDoc/setDoc</code> calls under the user's UID<br /><br />
              The app is fully structured for Firebase — all state flows through <code>userData</code> which is one Firestore document per user.
            </div>

            <div style={{ marginTop: 16, fontSize: "0.72rem", color: "var(--muted)" }}>
              80 questions from the official Hawaii Driver's Manual (2023 edition). Passing score: 70%.
            </div>
          </div>
        )}
      </div>
